	...
	function (request, sender, sendResponse) {
    	if (request.action == "returnItems") {
        	...
        	for (let i = 0; i < items.length; i++) {
            	const it = items[i];
            	htmlString += `<tr><td><a class="item-title" href="${escapeHTML(it.url)}" target="_blank">${escapeHTML(it.title)}</a></td><td><button class="delete-button" data-id="${it.id}">${trashSVG}</button></td><td><button class="finish-button" data-id="${it.id}">${checkboxSVG}</button></td></tr>`;
        	}
        	htmlString += "</tbody>";
