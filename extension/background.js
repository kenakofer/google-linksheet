...
chrome.runtime.onMessage.addListener(
	async function (request, sender, sendResponse) {
    	...
    	} else if (request.action == "getIsPageSaved") {
            console.log("getIsPageSaved message received.")
            const isSaved = await checkIfPageIsSaved(request.url);
            console.log(isSaved);
            chrome.runtime.sendMessage({ action: "returnIsPageSaved", isSaved: isSaved });
        }
	}
);
...
async function addPageToSheet(title, url) {
	...
	return await fetch(apiUrl, {
    	...
    	.then(data => {
            ...
            chrome.runtime.sendMessage({ action: "addedNotification", title: title, url: url });
...
async function checkIfPageIsSaved(url) {
    ...
    // console.log('Checking cache if page is saved:', cache_map.get(url));
    // console.log('Checking cache if page is finished:', cache_map.get(url).finished);
